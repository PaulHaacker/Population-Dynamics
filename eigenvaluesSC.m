% System w/ Self-Competition: Finding Eigenvalues by hand.
% Note: The workflow is almost identical. Differences: What we find in the
% 1st couple lines is not the s.-s. dilution, but the generalized s.-s.
% dilution, denoted gamma. The s.-s. dilution is nonunique and directly related to the
% (desired output setpoint) s.-s. output.
% Note: eigenvalues and eigenfunctions do not change.
...
%% ------ find equilibrium generalized dilution rate numerically w/o analytic solution
% of integral - provides a higher level of automation

% integrand of the Lotka-Sharpe condition
res_LS_int = @(a,gamma) k(a).*exp(-gamma*a-mu_int_fcn(a));

% Lotka-Sharpe in residual form as fcn of gamma
res_LotkaSharpe_fcn = @(gamma) integral(@(a) res_LS_int(a,gamma),0,A)-1;

u_0 = .5; % initial guess
gamma_num = fzero(res_LotkaSharpe_fcn,u_0);

%% plot Lotka-Sharpe residual fcn wrt. gamma

gamma_sample = 0:.1:10;
res_LotkaSharpe_sample = zeros(size(gamma_sample));
for ii = 1: length(gamma_sample)
    res_LotkaSharpe_sample(ii) = res_LotkaSharpe_fcn(gamma_sample(ii));
end

figure
plot(gamma_sample,res_LotkaSharpe_sample)
...
phi_0 = @(a) exp(-gamma_num*a-mu_int_fcn(a)); % eigenfunction of steady-state operator 
 % with eigenvalue = 0
